...
<body>
    ...
    <!--script>
        $(document).ready(function() {
            // Fetch data from the joins table
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(index, data) {
                        ...
                        var subCatClass = '';
                        if (data.subcat_id === '1' || data.subcat_id === '3' || data.subcat_id === '5' || data.subcat_id === '7' || data.subcat_id === '9') {
                            subCatClass = 'green';
                        } else if (data.subcat_id === '2' || data.subcat_id === '4' || data.subcat_id === '6' || data.subcat_id === '8') {
                            subCatClass = 'violet';
                        } else if (data.subcat_id === '0') {
                            subCatClass = 'red';
                        }

                        // Combine the logic to determine the final color class
                        var combinedClass = catClass || subCatClass;
                        var displayValue = (data.cat_id > 0) ? data.cat_id : (data.subcat_id > 0) ? data.subcat_id : '';

                        var row = `
                            <tr>
                                <td><input class="form-check-input" type="checkbox"></td>
                                <td>${data.period}</td>
                                <td>${data.contract_money}</td>
                                <td style="color:${combinedClass}">${displayValue}</td>
                                <td>
                                    <div id="big-dot" class="${combinedClass}">
                                    </div>
                                </td>
                            </tr>
                        `;
                        $('#resultWallet tbody').append(row);
                    });
                },
                error: function() {
    ...
    <script>
        ...
        //     // Fetch counter from localStorage or initialize it
        //     const storedDate = localStorage.getItem('uniqueDate');
        //     let counter = localStorage.getItem('uniqueCounter');

        //     // Reset counter if the date has changed
        //     if (storedDate !== currentDate || !counter) {
        //         counter = 1;
        //         localStorage.setItem('uniqueDate', currentDate);
        //     } else {
        //         counter = parseInt(counter, 10) + 1;
        //     }

        //     // Store updated counter in localStorage
        //     localStorage.setItem('uniqueCounter', counter);

        //     // Generate the unique number with padded counter
        //     const paddedCounter = String(counter).padStart(3, '0');
        //     const uniqueNumber = `${currentDate}${paddedCounter}`;
        //     document.getElementById('uniqueNumber').innerText = uniqueNumber;
        // }

        //  // Run the function once the DOM is fully loaded
        //  document.addEventListener('DOMContentLoaded', generateUniqueNumber);
    </script>
