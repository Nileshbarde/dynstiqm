...
<script src="wsscript.js"></script>
    ...
    <!-- <script>
        ...
        function startTimer(duration, display) {
            ...
            var countdownInterval = setInterval(function () {
                ...
                if (--timer < 0) {
                    ...
                    disableButtons();
                    // Show "waiting" status for the current period
        ...
        function displayWaitingStatus() {
            var resultSpans = document.querySelectorAll('.result-status');
        ...
        window.onload = function () {
            ...
            if (remainingTime && parseInt(remainingTime) > 0) {
                ...
                if (elapsedTime >= countdownTime) {
                    ...
                    var timeLeft = countdownTime - elapsedTime;
                            ...
                            userRecords.forEach(function(record) {
                                var recordRow = `
                                    <tr data-bs-toggle="collapse" data-bs-target="#r${record.id}" style="text-align:left">
                                        <td>
                                            ${record.period} 
                                            <span class="result-status" style="color:${record.status === 'success' ? 'green' : 'red'};text-transform:capitalize;margin-left: 20px;">
                                                ${record.status}
                                            </span>
                                            <span class="result-status" style="margin-left: 20px;color:${record.status === 'success' ? 'green' : 'red'}">
